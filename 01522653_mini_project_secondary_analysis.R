...
Ratio$SVL_dimorphism<- ifelse(Ratio$SVL_dimorphism<1, 1-Ratio$SVL_dimorphism, Ratio$SVL_dimorphism-1)
...
#check normality and autorcorrelation of traits 
hist(log(snake_dimorphism_data$SVL_dimorphism))
hist(snake_dimorphism_data$mean_lat)

#run basic linear model 
base_model <- glm(log(SVL_dimorphism) ~ mean_lat, data = snake_dimorphism_data)
# Inspect our linear model
summary(base_model)
#check residuals and plots
plot(base_model)#not bad can see two species with more leverage but expected because not many samples at some latitudes 
plot(density(base_model$residuals))
#plot base model
plot(log(SVL_dimorphism) ~ mean_lat, data = snake_dimorphism_data)
abline(base_model)
...
#take out tree node labels for comparison after
pruned_tree$node.label<-NULL
#plot tree
plotTree(pruned_tree,ftype="i")
#other option 
plotTree(pruned_tree,type ="fan",fsize=0.5,lwd=0.5,ftype="i")
...
#look for lambda
phylosig(snake_comp$phy, log(snake_comp$data$SVL_dimorphism), method="lambda", test=FALSE, nsim=5000, se=NULL, start=NULL,
         control = list())

#make pgls model
snake_pgls <- pgls(log(SVL_dimorphism) ~ mean_lat, data = snake_comp, lambda = "ML", bounds = list(lambda = c(0.0000000001, 0.001)))
summary(snake_pgls)
plot(density(snake_pgls$residuals))#still not good 
#plot model 
plot(SVL_dimorphism ~ mean_lat, data = snake_dimorphism_data, ylab = "Sexual size dimorphism index", xlab= "Latitude (degrees)")
